function handleCooldown(
  ...
  const cooldownKey = subcommandName
    ? `/${commandName} ${subcommandName}`
    : commandName;

  if (!client.cooldowns.has(cooldownKey)) {
    client.cooldowns.set(cooldownKey, new Map());
  }
  ...
  if (timestamps.has(userId)) {
    ...
    if (now < expirationTime) {
      ...
      return {
        onCooldown: true,
        message: `<@${userId}>, Por favor, espera hasta <t:${expirationTimestamp}:R> antes de usar el comando \`${cooldownKey}\` de nuevo.`,
      };
    }
  }
